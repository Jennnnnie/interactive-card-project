...
function Success(props) {
  return (
    <div className="complete-container">
      <img src={IconComplete} className="complete-img" alt="Complete Icon" />
...
function App() {
  ...
  function handleSubmit(e) {
    ...
    if (
      formData.expM < 0 ||
      formData.expM > 12 ||
      formData.expM.replace(/\s/g, "").match(/\D/g)
    ) {
    ...
    if (
      formData.expY.replace(/\s/g, "").match(/\D/g) ||
      formData.expY < new Date().getUTCFullYear() % 1000
    ) {
    ...
    if (formData.expY < new Date().getUTCFullYear() % 1000) {
      setErrors((prevErrors) => ({
        ...prevErrors,
        expY: "Year can't be blank!",
      }));
      isValid = false;
    }

    if (
      formData.expM - 1 < new Date().getUTCMonth() &&
      formData.expY <= new Date().getUTCFullYear() % 1000
    ) {
      setErrors((prevErrors) => ({
        ...prevErrors,
        expM: "Month can't be blank!",
      }));
      isValid = false;
    }
      ...
      <main>
        <div className="images-container">
          <div className="front-img-container">
            <img src={CardFront} className="front-img" alt="Card front"></img>
            <div className="front-content">
              <img src={Logo} className="card-logo" alt="Card logo"></img>
              <div className="card-front-text">
                <p className="img-card-number">
                  {formData.cardNumber != ""
                    ? formData.cardNumber
                    : "0000 0000 0000 0000"}
                </p>
                <div className="card-front-name-date">
                  ...
                  <p className="img-card-exp-m">
                    {formData.expM != "" ? formData.expM : "00"}
                  </p>
                  /
                  <p className="img-card-exp-y">
                    {formData.expY != "" ? formData.expY : "00"}
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="back-img-container">
            <img src={CardBack} className="back-img" alt="Card back"></img>
            <div className="back-content">
              <p className="img-card-cvv">
                {formData.cvv != "" ? formData.cvv : "000"}
              </p>
            </div>
          </div>
        </div>
        {isSuccess ? (
          <Success onClick={restart} />
        ) : (
          <form name="card-details-form" onSubmit={handleSubmit}>
            ...
            <div className="number-input-container">
              ...
              />
              {errors.cardNumber && (
                <p className="error error-cardNumber">{errors.cardNumber}</p>
              )}
            </div>
            ...
            <div className="date-cvv-input-container">
              <div className="date-input-contaier">
                ...
                {errors.expM && (
                  <p className="error error-expM">{errors.expM}</p>
                )}
                {errors.expY && (
                  <p className="error error-expY">{errors.expY}</p>
                )}
              </div>
